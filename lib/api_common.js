 ...
 * @param {Function} clearToken 可选的。清除全局token对象的方法，多进程模式部署时需在意
 */
var API = function (appid, appsecret, getToken, saveToken, clearToken) {
  this.appid = appid;
  ...
  this.clearToken = clearToken || function (callback) {
    this.store = null;
    callback(null);
  };
    ...
    if (token && (accessToken = AccessToken(token.accessToken, token.expireTime)).isValid()) {
      ...
      if (!retryed) {
        var retryHandle = function (err, data, res) {
          ...
          if (data && isBadAccessTokenErrcode(data.errcode)) {
              that.clearToken(function ( err ) {
                  that.token = null;
